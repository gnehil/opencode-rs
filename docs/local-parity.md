...
| Plugin runtime | `cli/cmd/tui/plugin/*`, plugin hooks | `plugin/*`, `session/processor.rs`, `message_handlers.rs`, `cli/mod.rs` | partial | External JS/TS plugins load via a node/bun subprocess bridge: local file plugins resolve and run; `event`/`config` notifications and `tool.execute.before`/`tool.execute.after` hooks delivered; plugins get an opencode SDK `client` resolved from their own `@opencode-ai/sdk`, pointed at the live server. Still missing: npm-package plugin install, remaining hook call sites (`chat.params`, `chat.message`, `permission.ask`, `command.execute.before`, `tool.definition`), TUI plugin slots |
