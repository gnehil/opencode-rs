...
| `run` non-interactive | `cli/cmd/run.ts` | `cli/mod.rs`, `session/processor.rs` | partial | JSON event stream now matches TS schema: `tool_use`, `text`, `reasoning`, `step_start`/`step_finish`, `error` wrapped as NamedError; per-delta and `tool_start` suppressed. All OpenAI-compatible providers (OpenAI, Azure, xAI, Groq, DeepSeek, OpenRouter, Mistral, Vercel, Cerebras, Copilot, DeepInfra, Fireworks, GitLab, LMStudio, Perplexity, TogetherAI, Venice) plus Anthropic and Google surface reasoning text on `CompletionResponse.reasoning` (via `reasoning_content`/`reasoning`/`reasoning_summary` for OpenAI-family, Claude thinking blocks for Anthropic, Gemini `thought` parts for Google). Streaming-side reasoning deltas not yet surfaced. Still missing: bus-driven `message.part.updated` event source, command/shell route shape parity |
| `run --interactive` | `cli/cmd/run/runtime*.ts`, `footer*.tsx` | `tui/*`, `cli/local.rs`, `cli/mod.rs` | partial | Multi-turn line-based loop wired (stdin → processor between turns, `/exit` to leave). Still missing: OpenTUI/Solid runtime parity — split footer, scrollback rendering, subagent frames, in-line permission/question prompts |
| `tui` | `cli/cmd/tui/*` | `tui/*` | partial | Worker/internal transport, session validation, model/agent pickers |
| `attach` | `cli/cmd/tui/attach.ts` | `cli/local.rs` | partial | `attach` subcommand still only validates sessions. `opencode run --attach <url>` now routes the prompt through a remote HTTP server: basic auth from `--password`/`--username` or env, session create/continue/select, SSE event stream, auto-respond to `permission.asked` (reject by default, accept under `--dangerously-skip-permissions`), text-part default output / raw JSON under `--format json` |
...
| Area | TS reference | Rust reference | Status | Next work |
| --- | --- | --- | --- | --- |
| Providers/model auth | `provider/*`, `auth/index.ts` | `provider/*`, `auth.rs`, `cli/provider_auth.rs` | partial | Stored OAuth `access` tokens resolve as the provider key; Wellknown credentials are exported into the process environment at startup so well-known/custom enterprise providers pick them up. Still missing: OAuth token refresh on expiry (lives in TS plugins, needs plugin `auth` hook integration), provider-side bearer/beta-header auth |
