...
| Plugin runtime | `cli/cmd/tui/plugin/*`, plugin hooks | `plugin/*`, `session/processor.rs`, `message_handlers.rs`, `cli/mod.rs` | partial | External JS/TS plugins load via a node/bun subprocess bridge: local file plugins resolve and run, `event`/`config` hooks delivered, `trigger_bridge` ready for call-site wiring. Still missing: npm-package plugin install, plugin→core SDK `client` callbacks, wiring `trigger_bridge` into every hook call site, TUI plugin slots |
