...
| `mcp` | `cli/cmd/mcp.ts` | `cli/mcp_cli.rs`, `mcp/*` | partial | Browser-open authenticate UX and unauthorized reconnect status parity |
...
| MCP runtime | `mcp/*` | `mcp/*`, `cli/mcp_cli.rs` | partial | Unauthorized reconnect status parity |
...
4. MCP unauthorized reconnect status parity.
