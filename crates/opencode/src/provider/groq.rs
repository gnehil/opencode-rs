...
lazy_static! {
    static ref GROQ_MODELS: Vec<ModelInfo> = vec![
        ...
        ModelInfo {
            ...
            interleaved: None,
            cost: Some(ModelCost { input: 0.0, output: 0.0, cache_read: None, cache_write: None, context_over_200k: None }),
            limit: Some(ModelLimit { context: 32768.0, input: None, output: 32768.0 }),
            modalities: None,
            experimental: None,
            status: None,
            provider: None,
...
impl Provider for GroqProvider {
    ...
    async fn complete(&self, request: CompletionRequest) -> ProviderResult<CompletionResponse> {
        ...
        let content = groq_resp.choices
            .first()
            .and_then(|c| c.message.content.clone())
            .unwrap_or_default();

        let tool_calls: Vec<ToolCall> = groq_resp.choices
            .first()
        ...
        Ok(CompletionResponse {
            content,
            tool_calls,
            stop_reason: Some(groq_resp.choices.first().map(|c| c.finish_reason.clone()).unwrap_or_default()),
            usage: TokenUsage {
