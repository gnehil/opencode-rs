...
lazy_static! {
    static ref MODELS: Vec<ModelInfo> = vec![ModelInfo {
        id: Some(ModelID::new("llama-3.1-sonar-large-128k-online")),
        name: Some("Sonar Large Online".to_string()),
...
impl PerplexityProvider {
    ...
    pub fn from_env() -> ProviderResult<Self> {
        let api_key =
            std::env::var("PERPLEXITY_API_KEY").map_err(|_| ProviderError::MissingApiKey)?;
        Ok(Self::new(api_key))
    }
}
