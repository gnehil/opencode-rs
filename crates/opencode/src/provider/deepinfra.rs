...
const API_URL: &str = "https://api.deepinfra.com/v1/openai/chat/completions";

lazy_static! {
    static ref MODELS: Vec<ModelInfo> = vec![
        ModelInfo {
            id: Some(ModelID::new("meta-llama/Llama-3-70b-chat-hf")),
            name: Some("Llama 3 70B (DeepInfra)".to_string()),
...
impl DeepInfraProvider {
    ...
    pub fn from_env() -> ProviderResult<Self> {
        let api_key = std::env::var("DEEPINFRA_API_KEY").map_err(|_| ProviderError::MissingApiKey)?;
        Ok(Self::new(api_key))
    }
}
