...
const API_URL: &str = "https://api.fireworks.ai/inference/v1/chat/completions";

lazy_static! {
    static ref MODELS: Vec<ModelInfo> = vec![
        ModelInfo {
            id: Some(ModelID::new("accounts/fireworks/models/llama-v3p1-70b-instruct")),
            name: Some("Llama 3.1 70B (Fireworks)".to_string()),
...
impl FireworksProvider {
    ...
    pub fn from_env() -> ProviderResult<Self> {
        let api_key = std::env::var("FIREWORKS_API_KEY").map_err(|_| ProviderError::MissingApiKey)?;
        Ok(Self::new(api_key))
    }
}
