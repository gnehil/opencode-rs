...
use crate::session::{SessionStore, PromptProcessor};
...
impl Tool for TaskTool {
    ...
    fn execute(
        ...
        Box::pin(async move {
            ...
            let task_id = params.task_id.unwrap_or_else(|| ulid::Ulid::new().to_string());

            let agent_info = get_agent(&params.subagent_type)
                .or_else(|| {
                    if params.subagent_type == "oracle" {
                        Some(crate::agent::AgentInfo {
                            name: "oracle".to_string(),
                            description: Some("Read-only high-IQ consultant for debugging and architecture".to_string()),
                        ...
                        Some(crate::agent::AgentInfo {
                            ...
                            description: Some("Multi-repository code search and documentation retrieval".to_string()),
                            mode: crate::agent::AgentMode::Subagent,
                            native: Some(true),
                            hidden: None,
                            top_p: None,
            ...
            let child_session = store.create(
            ...
            let result_text = match processor.process(&SessionID::parse(&child_session.id).unwrap_or_default(), &params.prompt).await {
                Ok(text) => text,
